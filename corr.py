...
# checking with Valid pairs and min_periods as Nan Values are also considered:
def plot_correlation_matrix(self,
                          df: pd.DataFrame,
                          columns: Optional[List[str]] = None,
                          min_correlation: float = 0.0,
    ...
    Returns:
        Dictionary containing correlation statistics
    """
    # Handle column selection with NaN check
    if columns is None:
        # Get numeric columns and filter out columns with all NaN
        numeric_cols = df.select_dtypes(include=np.number).columns.tolist()
        numeric_cols = [col for col in numeric_cols if not df[col].isna().all()]
    else:
        # If columns are specified, still check for all-NaN columns
        numeric_cols = [col for col in columns if not df[col].isna().all()]
    ...
    corr_matrix = df[numeric_cols].corr(min_periods=1)  # min_periods=1 allows correlation with some NaN values
    ...
    # Return correlation statistics (keeping original stats)
    try:
        correlation_stats = {
            'num_features': len(numeric_cols),
            'avg_correlation': float(np.nanmean(np.abs(corr_matrix_filtered))),
        ...
        # Fallback if there are issues with the statistics calculation
        correlation_stats = {
            'num_features': len(numeric_cols),
